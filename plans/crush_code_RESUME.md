...
| field | value |
|---|---|
| master | `eea60561a` (r88 filemap) → 20 picks `9bc98d6b8`…`06f658684` → stamps → filemap regen = ROUND-89-W1-TIP @ /home/sites/sugarcraft; CURRENT TIP = **`064430d85`** — branch new lanes off exactly this SHA — NEVER pushed |
...
### 2. ROUND-89 WAVE-2 — OPERATOR-AUTHORIZED 2026-09-17: EXECUTE THE §3 DISPATCH NOW (fresh session: you are the orchestrator; implement nothing yourself — delegate per APPENDIX I rules)
...
3. Restart recipe: fresh sandbox `git -C /home/sites/sugarcraft worktree add /home/sites/crush-lane-<id> -b lane/<id> 064430d85` + `php <worktree>/scripts/refresh-deps.php --mode=linked` (own copy — harness law; verify 19/19 + 8/8). Serials SYNCHRONOUS plain-pipe `env -u LINES -u COLUMNS` (NEVER tmux/setsid); K=8 gate standard; five-guard family filter in every brief; ledger law per-section re.split + assert-1 + numstat + far-canary; NEVER push; author `Joe Huss <[EMAIL]>`; welds re-derive from disk — briefs carry NO pre-baked lane SHAs.

### 3. READY-TO-DISPATCH LANE BRIEFS (fire BOTH in parallel at session start — disjoint files)

Both branches off master tip `064430d85` (re-verify with `git -C /home/sites/sugarcraft log --oneline -1` before branching — welds re-derive from disk; this dispatch base is verified current at brief-authoring time).

**y5 — E744 crush-host widget adoption** (branch `lane/y5`, worktree `/home/sites/crush-lane-y5`, artifacts `/home/sites/crush-r61-artifacts/y5/`). Full authoritative scope = backlog §E744 (`docs/plans/crush_code_hardening_backlog.md:20832-20834`) + triage row :208. Six workstreams; brief the builder to commit-EARLY one WIP commit per workstream:

1. **Cmd plumbing:** `Chat::delegateToInput` (sugar-crush/src/Chat.php:12123) currently DROPS the widget's returned Cmds. Consume `SetClipboard` via an OSC52 frame writer (CSI 52;p;b64 ST) on the frame channel with a size cap (lane picks N; over-cap = truncate + RuntimeNoticeSink notice, not silent). Any other widget Cmd arriving undealt MUST NOT silently vanish — lane rules between visible-notice vs pinned-discard-with-log, documents the decision in design.md.
2. **Key precedence:** Ctrl+C today intercepts into the quit flow BEFORE delegation. New rule: focused widget reports a selection → copy (TextArea copy arms shipped in r88 x5); no selection → EXISTING quit pins byte-unchanged. Pin BOTH polarities.
3. **PasteMsg routing:** consumed at Chat.php:1586 via `Chat::insertString` (E704 no-pre-trim law stays) — route to the focused widget when one owns input; else legacy path byte-identical.
4. **MouseMsg forwarding:** crush zone logic never forwards to ItemList; wire list-pane zone hit-test → widget y→row mapping (x5 proved the shape on candy-shell's WriteModel wire); document click precedence vs palette/menu bindings.
5. **LoadMoreMsg consumption:** y3's ItemList emits it when the cursor arrives on the last of visibleItems(); implement at least ONE real crush consumer pane + pin (source-of-next-page seam = lane's documented choice).
6. **View convergence:** crush paints its input box from Renderer (Chat.php:4231 comment), not `TextArea::view()`. Smallest honest step ONLY: adopt `TextArea::view()` at the delegated surface behind pins, or flag-gated; do NOT rewrite the renderer; document the ruling in lane design.md.

OWNERSHIP: sugar-crush/src/Chat.php + src/Renderer.php (+ src/Tui/** as needed). FORBIDDEN: src/Cli/Bootstrap.php (census-trio minefield — STOP-REPORT if it seems needed). This lane RE-PINS: new test files MUST get `scripts/parallel-tests-durations.tsv` rows IN-STEP (unlisted = zero shards); DocFigure arms + README/KeyBinding/Env/TrustKey doc rosters edited IN-STEP (guards live in sugar-crush/tests/Config + tests/Commands/KeyBindingDriftTest); OSC52 raw-ANSI writer must conform to the NoRawAnsiInTranscript derived census (flip IN-STEP with its own write site — study the existing writer roster first); prefer RuntimeNoticeSink over new stderr sites (StderrEmitterCensus five-file set). beginTurn wiring: E199 seam at Chat.php:8266 scheduleBackendCompletion (per fn-seam record). Gates: targeted families + five-guard 125/7,362 + Config window (read live count) + FULL SERIAL with +ΔT reported for the merge agent (floor 12,027 + new tests EXACT), K=8 shardcheck, tools rc=0, config md5 d96e124ee7967eb34ef479ef824231ad start==end, keystones on plain pipe (TerminalSizeFallbackIsolationTest reddens under a PTY ctty; CallbackAuthoredRefusalTest known ~1-in-3 in-shard flake — re-run that one before declaring). No shared-loop timers without LoopPin (crush bootstrap already pins it).

**y6 — shine 7.2 emoji-shortcode map** (branch `lane/y6`, worktree `/home/sites/crush-lane-y6`, artifacts `/home/sites/crush-r61-artifacts/y6/`). Row: `findings/plan_candy-shine.md:672` “| 7.2 | Limited emoji map | MEDIUM | PENDING |”. Build the map to upstream-glamour parity; vocabulary DUPLICATED per the r89 no-glow-require cycle ruling (reference copy: sugar-glow's GlamourTheme emoji table; composer.json STAYS UNTOUCHED). Fold row 8.1 (emoji regex compiled every call — same file region) if natural; else stamp 8.1 with why-not. Tests: byte-identity of representative docs through render() before/after map-widening on known-shortcode fixtures, unknown shortcodes pass through verbatim, multi-codepoint sequences preserved. Figures: shine 573/1,075 baseline (lib-local; no crush re-pin if zero crush files — verify), glow 122/313 UNMOVED, crush `Shine|Glamour|Style` window 24/813 unchanged. Stamp row 7.2 (+8.1 if folded) ✅ IN-PLACE (audit-walkthrough law; original text preserved with Was: line). Alternative ONLY on new operator ruling: ⏭ decline-with-why (then E736 closes ruling-only) — default is BUILD.

### 4. WAVE-2 REVIEW + WELD ROUTE

- Land-gate every builder report before review (4 commands: `git rev-parse lane/<id>` + `git cat-file -t <tip>` + `.git/worktrees/` entry + artifacts dir non-empty). Blank/identical report → resume SAME task_id ≤10× then fresh re-cut SALVAGE-FIRST step 0.
- Reviews = task+subagent_type=coder, fresh linked review worktrees (exec the worktree's OWN scripts/refresh-deps.php — main-tree copy mislinks, r89-rv-y3 lesson); reviewer re-runs gates independently + owns its own cp+md5 mutation set; evidence `/home/sites/crush-r61-artifacts/r89-rv-y{5,6}/REVIEW.md` verdict-first ≤25 lines.
- Weld order: **y6 first** (lib-local, zero crush files → floor exact-carry, trivial), then **y5** (RE-PIN domain: pick → guards → serial green → hand merge agent re-pins README headline + suite-figure.json + durations rows + any DocFigure flips; K=8 conservation vs re-pinned figure; cadence per lane-ce/r76 law: hand-bump tests-figures ONLY, green serial, refresh-suite-figure rewrites both, confirm).
- Stamps at weld: backlog §E744 heading [OPEN]→[CLOSED] + CLOSED paragraph citing pick SHAs re-derived from git log; §E736 → [CLOSED] if y6 landed (census 0 = PLAN COMPLETE; MATCHUPS backlog sentence: residual port-completion is the MATCHUPS board itself, not this ledger) — ledger law: per-section `re.split(r'(?m)^(?=### E)')`, assert-exactly-1, post-edit numstat, far-canary grep of a distant surviving phrase (this campaign's history: 2026-09-13 3,837-line wipe caught only by numstat — lane-lc law).
- Triage table rows flip in-place; the census awk (§1) must then print NOTHING (census 0) — capture output verbatim into the worklog section.

### 5. DEFINITION-OF-DONE (campaign completion)

ROUND-89 closeout commit(s): worklog `## ROUND 89` wave-2 addition (keep wave-1 content, append), RESUME banner+§0-NOW-93 → final §0-NOW-94 “ROUND 89 CLOSED — CENSUS 0 — SUGAR-CRUSH HARDENING PLAN COMPLETE” (state table final tip/floor/figures + full-sweep chronology pointer; supersedes this §0-NOW-93 — wave-1 CLOSED @ tip `064430d85`), filemap regen (tools per APPENDIX III), root crush_code.md pointer flip, cleanup: `git cherry` verify then remove ALL lane worktrees/branches + merge worktrees (expect porcelain-clean; vendor symlinks ignorable), final `git worktree list` = main + foreign sugarcraft-cc, porcelain empty, NEVER pushed. Evidence `/home/sites/crush-r61-artifacts/closeout-r89/REPORT.md` verdict-first. Trigger-watch roster survives §0-NOW-94 verbatim (watches are NOT rows).
...
**The headline: ROUND 87 executed the r86-approved tail cut — three lanes, all reviewed APPROVE-class before merge (w2 carried one MAJOR through a full fix + re-review cycle), six picks ZERO conflicts (file-disjoint), ZERO sugar-crush files moved → floor exact-carry for the FOURTH consecutive round.** w1 built the candy-vt tail (4.1/12.3/4.3/4.4 API batch, 6.1/6.3 perf, 2.3 DRY) and RULED 6.2 gated-no-build by consumer sweep; 6.4 stays deferred-with-trigger (needs a new public Buffer row-shift API). w2 shipped sugar-bits 2.4 (`Tabs::view()` consumes stored scrollEnd) through the MAJOR-1→POLICY-G history: left-ellipsis over-claim caught in review, fixed with a single-parity `rendersLeftEllipsis()` seed, proven by a 15,506-row tuple-paired audit (improve=46, regress=0) and a WALK-FORM pin (the withActive-jump form demonstrably missed the seed-neuter mutation). w3 dispositioned sugar-charts Phase-5 4/4 (5.1 verified-landed; 5.2/5.3/5.4 ruled-skip — feature/new-dep/API-sprawl) and hardened the Sixel memo key (rv-v4 MINOR-1, tests-only). The rv-w1 seam minted §E742 (Theme cube palette mis-alignment — frozen-not-blessed by w1; needs a correct-to-xterm vs document-fallback RULING). Defect queue: ZERO defects — both survivors are decision-shaped.

### 1. THE FLOOR — THE ANCHOR, AND ITS DOMAIN

| figure | value | domain |
|---|---|---|
| **ANCHOR (carried)** | **12,027 / 170,392 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0** — serial green x2 at r86u `988696aea`, PLAIN PIPE, linked, cwd = sugar-crush; r85 both waves, the r86 weld AND the r87 weld moved ZERO sugar-crush files → carried EXACT, no re-pin; K=8 conservation +0 | **UNCHANGED since r86u absorption — link-domain; ±32A cold/warm `ensureFixtureRepo()` wobble band still the assertions truth** |
| lib figures at close | candy-vt **926 / 12,689** (+14T/+46A w1, render-dump md5-identical `dccc5c4857103432105e314a0fcc1209`) - sugar-bits **514 / 1,085** (+5T w2; TabsTest 60/108; sweep committed md5 `c6cf5817`) - sugar-charts **577 / 1,453** (+1T/+8A w3) - consumers UNMOVED: pty 663/1862/17S · vcr 948/4082/14S · forms 1891/3164 · prompt 158/310/2S · glow 122/313 · stickers 221/476 · query 1483/4203 · tick 144/349 · boxer 187/375 · sprinkles 751/2629 · shell 334/694 · layout 157/882 · crush window `Vt|Terminal|Buffer` **305 / 21,034** · five-guard **125 / 7,362** | lib-local; every touched lib gated FULL + both repo tools rc=0 at the weld |
| chain | `b26b9989f` (r85 filemap) → r86 weld `cfb26f8f2` (floor exact-carry) → picks `f48537daf` `c37486747` `848f1fbb0` `232686276` `66debe01c` `971c0fa96` → companion `7988f2305` (E742 MINT + folds) → stamps + worklog + `§0-NOW-90` + pointer → filemap regen = ROUND-87-TIP | NEVER pushed |
| durations | **532 rows HELD** — no new sugar-crush test files (sibling-lib files never belong in the tsv) | shared infra |
| config | at-rest truth `d96e124ee7967eb34ef479ef824231ad`, start==end at the weld | linked-domain |

### 2. DECISION POINT — CAMPAIGN MECHANICALLY COMPLETE (operator call m0225); ROUND 88 ONLY ON OPERATOR DIRECTION

**(a) Row census = 2 survivors** (command RE-RUN at the filemap regen; output verbatim):
...
OPEN  **E736** 
OPEN  **E742** 
$   # 2 survivors: E736 (umbrella — ALL remaining shape is ruling/feature, every tail row dispositioned at r87) + E742 (cube palette — RULING required before any build)
```

**(b) THE SURVIVORS (none is defect work):**

1. **§E742** — candy-vt Theme cube palette mis-alignment: orchestrator RULING correct-to-xterm (user-visible; vt goldens + ansi-index census ripple + consumer snapshots) vs document-fallback (record-only + KNOWN-DIVERGENT pin). Posture: NOT auto-built (behavior-change STOP).
2. **§E736 tails, all trigger-gated:** forms Phase-5 (DECLINED wholesale r86-v6 — reopen ONLY on real consumer demand); vt 6.4 `scrollUpOne` (DEFERRED-WITH-TRIGGER — needs new public Buffer row-shift API, own ruling on demand); shine/layout/bits/charts declines ride the row; MATCHUPS 🟡/🔴 port completions = full add-a-lib checklist per lib, one lib per wave, only when scheduled by the operator.
3. **Phase-12 stays PAUSED** — never auto-resume (standing user decision).

**(c) TRIGGER-WATCH ROSTER (watches are NOT rows):** carried from §0-NOW-89 EXCEPT two retirements — **rv-v4 sixel single-channel key-collapse leg CLOSED by w3** (the memo key now requires every channel — pinned by blue-only/green-only pairs, M1/M2 each redden exactly it) and **rv-v1 era-figure seam CLOSED by w1 in-lane** (305 not 288). Still watching: CallbackAuthoredRefusalTest in-shard flake; candy-freeze zsh asymmetry; JsonStreamBackend flush-door; wish stale README badge; q15 DECSCUSR re-verify; q10 carrier-list arm; pa paste-while-modal pin; ReadPathCensus backslash blind spot; pf maxOutputTokens threading seam + ClaudeCode ceiling no-op; E696-deny-residual; updateRegistration() redirect-churn; E309 DenialKind decision; E611 tripwire; E694 step-3 / E25 re-severity gate; E655 stays VOID; LspClientDispatchPumpTest ambient; Chat.php:8266 notice-drain seam; rv-v2 Expression mutable-`$terms` freeze-seam; rv-v3 Confirm shared-carrier fold-when-possible; rv-v3 Input::withValidator/mutate trait-drop family STILL LIVE (Phase-8/2-domain); rv-v5 render-dump arg shape; rv-v5 vim j/k product ruling; bits Select workerPool stores-dead asymmetry; r87 adds: **w2 zone-cost ≈13 cells law** (measure stripped rows for any Width::of geometry pin).

### RESTART INSTRUCTIONS — the new-session startup recipe

**Tip chain:** this closeout's docs commits (companion/stamps/worklog/RESUME/pointer + filemap regen) sit atop the six picks + `7988f2305`; master is fast-forwarded to the final closeout tip. NEVER pushed.

1. Read this §0-NOW-90, the worklog `## ROUND 87` section, and `docs/plans/crush_code_filemap.md` (2 rows = census: E736 + E742).
2. Preflight in any fresh sandbox: `git -C /home/sites/sugarcraft status --porcelain` shows ONLY ignorable operator dirt at the tip; `php scripts/refresh-deps.php --mode=linked` FULL then EXPLICIT census **sugar-crush 19/19 + candy-pty 8/8**; config at-rest truth `d96e124ee7967eb34ef479ef824231ad`; floor gate if doubted: K=8 `bash scripts/parallel-tests.sh --durations scripts/parallel-tests-durations.tsv --out <fresh>` (~85s) or plain-pipe serial (cwd=sugar-crush).
3. **DO NOT auto-open round 88** — the campaign is mechanically complete per the operator (m0225). If directed: the §2 menu is E742-ruling-first (one small lane either way), then trigger-gated E736 tails only with real demand. NEVER push; author `Joe Huss <[EMAIL]>`; no composer, no caliber, no touch of `.mcp.json`/`.sugar-crush/config*` operator files or foreign `sugarcraft-cc`.
4. Per-lane loop unchanged (task+coder implement → SHA/worktree verify → task+coder review → fix round → weld; blank/fragment reports → RESUME same task_id ≤10x with disk-probes, salvage-first before any redo).
5. If a future round welds: close as rounds 83–87 did — worklog + stamps + `§0-NOW-91` + pointer + filemap regen; re-pin crush figures ONLY when sugar-crush files move (four rounds now proven on the zero-move EXACT-carry shape).
