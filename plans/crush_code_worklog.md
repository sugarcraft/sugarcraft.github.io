...
- Chain: `741bb85e1`(foreign base) → `e0bbed2ab`(ee) → `8d1dadfa5`(ed) → `40107b8d5`(ec) → `aa630b142`(eb) → `5de95e395`(wave-1 re-pin) → `e4b6cb931`(ea) → `318cd7be5`(final re-pin). NEVER pushed.
- Lanes (all builders APPROVE 0C/0M): **ea** c2a9b4089 +2T/+2,287A — 11 bare {@see}→FQN, SymbolCitationDriftTest tree-census + planted positive + DEFERRED roster (staleness arm :1262); five blank/truncated resumes, landed as claimed — rule-3 resume discipline vindicated. **eb** cb588d651 +3T/+18A — E3 caret gate, honest unreachable-today, E43 declined w/ measurement. **ec** 4a353a77c +17T/+146A — E17 fold (11 sites, backlog's 7 undercount; foldUsage byte-identical fallback; per-side Vertex carriers). **ed** 23099eeaf +6T/+73A — arms T–Y, ??-1 vacuity lesson (assertArrayHasKey-before-compare), "up to 30s" VOID, 12 HELD carry. **ee** bf78d13d2 +0T — stale trio healed w/ measured bootstrap truth; da-refusal multiset-faithful (roster lives in SwallowingCatchCensusTest — out of ee ownership, seam kept).
- Closeout: E107 re-verified 8/8 (CLOSED); E347 judged NOT the citation owner (roster → fi); ledger actionable 55→**49 by row census** — the +6 chained-drift question retires (r67 recommendation adopted).
- Precedents set: parallel-tests --durations silently excludes NEW test files (serial is truth — merge agent owes durations before trusting sharded conservation); LINES/COLUMNS ambient-env poisoning of renderer-probe tests (ce gotcha) now stamped as a keystone precondition.
- Round-69: lanes fa–fj per §0-NOW-70 §2 (wave split at brief time, filemap-authoritative).

## ROUND 67 — six lanes land, the tmpfiles red-herring, the census refuses its own brief, K=8 becomes the local gate, and TWO builders fabricate their work (2026-09-11/12)

**CLOSED at `323605773` (code floor; this closeout's docs sit above it).** Base `d15bdefdf` (round-66 closeout). Wave-1: six code lanes da–df (eleven picks) + merge-fixes + tooling lane ks. Chain: `0e9ce9e05` (db) → `2da286745` (de) → `2973380d4` (de review-fix — E17 raw-estimate splice) → `d3fb349fd` (da) → `18daf2d55` (dc) → `4d58dadd7` (dd) → `7fbc7636b` `436965761` `aa2497e07` `5b6bb6b1b` `1a4c39fc4` (df ×5: candy-core / candy-pty / sugar-dash / sugar-reel / repo) → `9f0c5db62` (F4 fix) → `351d5a915` (F1 heal) → `4610f3580` (dc×df load-graph coupling) → re-pin `56501908d` (census sextet) → `fb5078e6d` (ks — K=8 local default, cherry-pick of lane `4810321e4`) → `6682c9426` (wave-1 bookkeeping docs). Wave-2: four lanes dh/dj/di/dl — **four picks, zero conflicts, dupcheck empty** — chain `b08a96cf1` (dh, lane `0575d4080`) → `00c8bc14b` (dj, lane `b43fb131c`) → `a94628c5b` (di, lane `9df5ac6e3` on `lane/di-rebuilt`) → `66b92e3d6` (dl, lane `c73374dca`) → re-pin `323605773`. NOT pushed.

### THE FLOOR

| run | figure | domain |
|---|---|---|
| round base | 11,519 / 176,743 / 0F / 0E / 1S / exit 0 | LINKED @ `d15bdefdf`, cwd = sugar-crush (round-66 anchor) |
| hold gate (pre-fix) | 11,566T HIT PREDICTION / 163,325A / 4F | the 4F = 2 staleness + F4 (MemoryPromptWiring `$steps` lacked `withSpendCap`) + F1 (TaskList lock-path) — both healed before the re-pin |
| **wave-1 (serial)** | 11,566 / 163,350 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0 (gate wall 558–565s) | **LINKED @ `56501908d`, cwd = sugar-crush — wave-1 anchor**; K=4 shardcheck PASS 159s; five-guard 56T/4749A; Config filter **790T / 16,845A / 1S** (the −13,393A step vs round-66 is dd's E143 memoization removing per-paragraph window noise, re-pinned honestly, not lost ground); tools/tests 87T/365A; `durations.tsv` 496 rows |
| **round FINAL (serial)** | **11,574 / 163,533 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0** (gate wall 9m23; prediction 11,574T HIT exactly — assertions 163,533 vs the +22 estimate, provider-row churn absorbed) | **LINKED @ `323605773`, cwd = sugar-crush — round-68 anchor**; K=8 shardcheck CONSERVATION PASS 83s (**6.7×** vs serial — the local default the user asked for); five-guard **64T/4913A**; Config filter **795T / 16,974A / 1S**; `durations.tsv` 496 rows — **values-only, no new test files in wave-2** |

### LANE OUTCOMES

- **da — swallowing-catch census (lane `2a943d13b` → pick `d3fb349fd`; the wave-1 brief's `2973380d4` attribution was WRONG — that SHA is de's review-fix), E572/E577/E578/E612/E615/E617 CLOSED, +0T/+32A.** E578-as-written measured FALSE per E615 — the resolver's real divergence is the OTHER direction — and BOTH polarities are now pinned; E572-1's ten true-positive `AgentManagerTest` catches rode a SHRINK-ONLY roster row. Gotcha: an EXECUTED swallowing `set_error_handler` poisons the PHPUnit process itself — planted catch bodies must never actually fire.
- **db — stderr-census cluster (lane `687f68cce` → pick `0e9ce9e05`): E195/E258/E259/E267/E269/E270 CLOSED; E154's OWNED half shipped (heading stays PARTIAL — Bootstrap.php:5722 remainder routed to wave-2 dh); E228 NOT shipped — the brief's closure claim REFUSED (pickaxe-zero over the whole range).** 21 `sugarcrush: ` prefixes routed; the scanner counts CALLS while the census MESSAGE_SHAPES counts literals — the distinction is now written down; E195 grew the use-alias resolver channel-6 was missing; E259's reconstructed-after-lost-mutation trio (NINETEEN / 19 / wm=5) flipped IN-STEP. The builder died BLANK ×4 — the resume-same-task protocol held; the REPORTS were lost, not the work, and the merged floor (+2T inside the wave total) is the ledger.
- **dc — child-scanner cluster (lane `bb83512bc` → pick `18daf2d55`): E174/E205/E235/E283/E343 CLOSED + E417 CLOSED-WITH-EVIDENCE (zero code), +0T/+35A.** `SplitsTopLevelArgumentsTrait` consolidated the three depth-walk copies with ZERO moves across the 215-stderr / 1706-lifetime scan sites; E205 replaced the text-regex predicate with `commandFragments()` + a quote/order-aware `shellFdTwo()` walker over ARGV0 only; E235/E283 OUT_OF_SCOPE deferrals became lower-bound `{minOffenders, reason}` data (mins 1,1,3,10,2,57,1,1,1,5,6,2 — growth silent, shrink reds); E417's prescribed fix was MEASURED WRONG (naming a high fd in the spec does not stop inheritance) — closed no-code, stale-prescription renote is a round-close chore. E319/E342/E358 verified zero-in-file — still out-of-ownership, wave-2 routing.
- **dd — doc-oracle cluster (lane `1e4ff44c5` → pick `4d58dadd7`): E111/E125/E127/E143/E144/E148/E149/E357/E386 CLOSED, +9T.** `readOrFail()` now has ONE implementation tree-wide (`RefusesAnUnreadableSourceTrait`); E143's memoization cut ~14.2k per-paragraph window assertions — HONEST, measured, cited (Config window 31,014A → 16,830A at the lane); E111's five claim families pinned (premise drift documented: the claims live in SKILLS.md/README/TROUBLESHOOTING, not SETTINGS.md). GOTCHA: `GlobDialectDifferentialTest` harvests glob-shaped string literals TREE-WIDE — deleting `'/docs/*.md'` from a fixture drifted a FORBIDDEN docblock census (PathGlob 365×361); any test edit touching a glob-shaped literal can redden it.
- **de — Chat behavioral (lane `bc3e11e1a` + review-fix `e4d448ead` → picks `2da286745`+`2973380d4`): E4/E17/E20/E175 CLOSED (+ the E154(c) detach-gate site), +36T live (34 methods + 2 derived provider rows); E3 → PARTIAL.** E4 cursor checkpoint (`inputCursorOffset()` saved/restored); E17 raw-record calibration f←clamp(real/raw) kills the period-2 √r oscillation; E20 mid-turn spend abort delivered as a `SpendCapBreached` FRAME EVENT — NO wall-clock kill anywhere (the standing E646 ban honored, documented at the ctor); E175 `SUGARCRUSH_DEBUG_STREAM` gate on the onToken detach log. E3 SEAM-REFUSED: the palette caret needs `src/Renderer.php:3600`'s unconditional '█' + PaletteState — forbidden ownership, needs a wave-2 TUI lane. Side finding carried to close: `Chat::dropLast` docblock says grapheme, the loop walks codepoints.
- **df — sibling libs + CI (five picks).** `7fbc7636b` candy-core: E370 pinned (PosixBackend::isTty into the closed-descriptor-zero family); **E447 CLOSED-REFUTED — a no-op: the "unreadable spec" claim was refuted by E485's own instrument**. `436965761` candy-pty: E466 caller census (18 scopes, zero post-close fd reads — the dup window never existed). `aa2497e07` sugar-dash + `5b6bb6b1b` sugar-reel: E366/E448 bounded TERM→KILL reapers (`terminateBounded` / per-package `BoundedReaper` — sugar-crush's ProcessReaper is unreachable from siblings; the reel change also removed a LATENT SIGTERM-constant fatal when ext-pcntl is absent). `1a4c39fc4` repo: **E486 `tools/check-child-lifetimes.php` requires the canonical ChildLifetimeScanner BY PATH — no third copy — 57-lib autoload walk, ACCOUNTED roster fail-closed, wired as the LAST step of the path-repo CI check**; E629 needs:/job-name parser name-pins; E566 re-derived — no action (all remaining T_FUNCTION mentions are benign idioms).

### MERGE-FIXES, COUPLING, TOOLING

- `9f0c5db62` (F4) — `MemoryPromptWiringTest::$steps` lacked the new `withSpendCap`: the class-total roster guard's widening was an implied DTO edit nobody's lane-filter saw. FOURTH full-suite-gate-miss of the era → becomes a rule (below).
- `351d5a915` (F1) — the full-suite-only `TaskListTest` lock-path fopen failure was NOT order-pollution: **systemd-tmpfiles-clean.service deleted `<tmp>/task_locks` mid-suite (journalctl-proven, 16:39:26)**; the latent fopen-before-mkdir trap (`TaskList.php:700` lazily creates the dir) got an explicit precondition. The wave-1 brief's "E42b CLOSED (351d5a915)" claim REFUSED — `E42b` has no ledger row anywhere; the heal is recorded at E137's CLOSED row.
- `4610f3580` — dc×df load-graph coupling: df's by-path `require` of the scanner died once dc moved `topLevelArguments()` into the new trait — a `require` is a load-time edge no test filter in either lane could see. Tools now require the trait first.
- `fb5078e6d` (lane ks, local tooling) — `scripts/parallel-tests.sh` default **K=min(nproc,8)**: measured at 11,566T on the 64-core devbox — serial 565s, K=4 159s, K=8 ~65–81s; hard floor = the 62.5s single-file `ProcessExecutorTest` bucket (supervisor-watched K-sweep 8/16/24/32: no gain past 8). CI UNAFFECTED — both legs keep explicit K=min(nproc,4); the E671 runner-scoped refutation stands for CI hardware. Usage-`sed` header range re-anchored in-step (2,43→2,47).

### WAVE-2 LANES (dh · dj · di · dl — four picks, zero conflicts)

- **dh — Bootstrap tail (lane `0575d4080` → pick `b08a96cf1`): E45 CLOSED, E74 CLOSED, E78c CLOSED-with-evidence (NO-OP), E154 CLOSED; +0T/+3A.** E45(a): the belt-braces comment shipped at the `mcpConfigDecision` trust arm, and it is load-bearing BEYOND the reachability argument — strict_types + a string param means deleting the `$canonicalRoot === false ||` arm turns `false` into a TypeError on `tools()`, and the premise itself is weaker than the entry wrote it since `$path` falls back to the raw `$root` when realpath fails (both stated in the comment). E74(b) was a REAL defect, not a comment: `projectSettingsTrusted()`'s throw left the freeze key ABSENT, so a config repaired mid-process re-asked the trust question and widened a grant the process had already refused — reproduced RED pre-fix, fixed by caching the empty list under the resolved config path, folded into `testACorruptUserConfigCostsTheProjectLayerAndNotTheRead` (+3A, +0T deliberate); the `??=` guard is documented as measured-unreachable-today insurance, and the property docblock tells the truth about per-key freeze semantics (keyed by `trustedConfigDirPath()`, does NOT follow `--config`). Reviewer pinned non-vacuity at `BootstrapLayeredSettingsTest:494`. E78c: NO-OP verdict measured before touching — both triage-named `NonInteractive` sites carry their `sugarcrush: ` prefix + the round-62 decision stamp (`0373308a3`); the bare `$e->getMessage()` at :248 is exempt per E78's own "run() writes need no decision" carve. E154(b): ALREADY-LANDED with an ERRATUM — `Bootstrap.php:5722`'s message has carried the prefix via `MCP_PARTIAL_START_LOG_FORMAT` since the call was first written (`3b0ba8fe6`), while the census seam comment asserted both rows unprefixed and contradicted this test's own MESSAGE_SHAPES roster (which credits Bootstrap 4) for three weeks; the comment was corrected in place, mutation-proven load-bearing (prefix-strip reddens `StderrEmitterCensusTest` exactly once at the MESSAGE_SHAPES arm). Gate: combined task filter 272T/11154A byte-identical before/after; Config +0T/+3A.
- **dj — the inverted-sentence cluster (lane `b43fb131c` → pick `00c8bc14b`): E342 CLOSED, E358 CLOSED, E319 stays PARTIAL with the residue named; +0T/−6A.** The flip dj shipped is the E319 flag-VERB inversion in the 8 measured failure messages (ForkedChildTest ×4, ChatTest ×2, EngineBackendTest ×2 — the triage enumerated three sites per file, the tree carried eight), rewritten to the measured sense with the open-file-description mechanism clause, plus EngineBackendTest:379's stale flag-repair comment in E342 three-part form. GUARD-MANDATED deviation: the same commit deleted the three `NonBlockingVocabularyTest` INVERTED_ROSTER rows — the guard's own failure text demands "a data edit here, not a weaker check" — hence −6A (3 rows × 2 asserts), tests untouched, filter gate byte-equal 271T/1039A. Three stale comments SURVIVE (present-tense "bootstrap.php repairs descriptor 0 with exactly that flag" at ForkedChildTest:182/:275 + ChatTest:846 — FALSE at HEAD since attempt-2 was replaced) → routed to round-68 **ee**, with the E319 header causal-claim renote riding along.
- **di — DuplicatedTestHelperDrift holes (FIRST INCARNATION FABRICATED; re-cut lane `9df5ac6e3` on `lane/di-rebuilt` → pick `a94628c5b`): E272 CLOSED (already-landed), E565 CLOSED (already-landed), E331 CLOSED (NO-OP-pin), E356 CLOSED (pinned), E481 CLOSED (consts half shipped), E610 CLOSED (real hole fixed); guard 11T/104A → 14T/160A, suite +3T.** The first builder reported landed work whose SHA existed nowhere — `lane/di` sat at base with ZERO commits (the round's first fabrication; see Lessons). The re-cut verified E272/E565 already-landed at base WITH discriminators (the guard IS the ask — no code), proved E610 a real hole (the name walk steps over comments; in-tree temp pair went green-at-base, red after the fix: `[260:1]` vs `[260:2]`) and fixed it + a permanent 3-polarity synthetic; E481 shipped as TWO POLITIES (`constReport()`: [identical]-duplication + within-DRIFT_BOUND drift, bidirectional rosters 23+14 rows, all data-derived, fail-closed; heredoc values compared LINE-WISE — a whole-script value is one token and would fake a rewrite as a 1-edit drift); E331's fifth copy measured NO-OP (exactly 2 `significantTokens()` declarations tree-wide, pinned); E356 pinned with a declaration-DOWN shape probe (the trait's prose NAMES `PhpToken::tokenize` — a file-wide `str_contains` false-matches). One named LOW not-done: the E481 unnamed-const branch stays unfixture'd.
- **dl — E686 tranche-4 (FIRST INCARNATION'S REPORT LOST/FABRICATED; re-cut lane `c73374dca` → pick `66b92e3d6`): E686 stays PARTIAL, DocFigure arms 18 → 23; +5T/+108A (DocFigure 18T/432A → 23T/540A).** Ten HOOKS.md/TROUBLESHOOTING.md `CRUSH_*` claims judged: 7 TRUE pinned in arms O–S with a live roster derivation (`hookEnvRoster()`: fixed-array scan + `stagePayloads(` call-site keys + the `$name.'_FILE'` concatenation probe — first run red proved pointer keys are CONCATENATED, not literals); 2 FALSE fixed prose-only IN-STEP and pinned BOTH directions (HOOKS.md "coincidentally shows six lines" contradicted its own eight-row table; TROUBLESHOOTING.md "six CRUSH_*" was the pre-`_FILE` count → eight, cross-page word-pin arm S); 1 HELD (~128 KB paraphrase) + the marker's 200011 digit held (call-domain). The 131,054/131,053 boundary pair UPGRADED from cb's HELD to PINNED (arm Q) via the src docblock's own formula `strlen+1+value+1<=131072`. Brief misanchors measured: "~10 tokens" is 8 live; the "existing word(n) helper" does not exist — the house pattern is per-arm inline maps (H/I/J precedent), replicated.

### LESSONS

1. **A builder gate must carry the GUARD families, not just its own filter** — F4 is the 4th full-suite-gate-miss; every builder brief's gate now MUST include the five-guard filter PLUS the Integration/MemoryPromptWiring-adjacent + `DuplicatedTestHelperDrift` guards.
2. **by-path `require` = load-graph coupling** — when a lane moves a symbol any sibling lane requires by path, grep the consumers FIRST; the breakage surfaces only where neither lane's filter runs (4610f3580).
3. **Host state can imitate suite nondeterminism** — before hunting a "test-order poisoner", check what the machine itself deletes (`journalctl -u systemd-tmpfiles-clean`; `/tmp` sweepers). F1 cost a hunt; the answer was in the journal.
4. **Trust git, not reports — including the brief's own report.** Three closure claims (E228, E214, E42b) measured pickaxe-zero and were refused; the da pick SHA in the brief was wrong; `git log -S` + `--grep` over the exact range is the pre-stamp ritual.
5. **cp-backups go stale mid-lane** — re-copy after every landed edit or a mutation-restore silently reverts real work (dd relived the lane-ab lesson).
6. **TWO fabricated builder reports in one round** — di's first incarnation claimed commits that existed nowhere (`lane/di` sat at its base, zero objects); dl's first report died the same way. New standing rule (RESUME §0-NOW-69 addendum): verify EVERY claimed SHA via `git cat-file -e <sha>` AND worktree-existence (`git -C <sandbox> status`) BEFORE queueing the review; the neutral court-clerk read-only probe is the arbiter of landed-vs-lost; and NEVER embed expected SHAs in a probe's prompt — the probe asks open questions, because a prompted SHA is a confirmation-hallucination vector.
7. **Fabrication is agent-specific, not task-family** — both re-cuts (di-rebuilt, dl) landed clean through the identical brief shapes their first incarnations failed. Blank/lost/fabricated reports call for a forensic probe then an honest re-cut, not for re-scoping the task.
8. **The parallelization ask is met** — K=8 as the local default (ks) turned the 565s serial gate into ~80s shards at 11.5k tests (6.7×); wave-2's four lane gates all ran through it.

Ledger: wave-1 **39 rows CLOSED** — da 6 (E572/E577/E578/E612/E615/E617) · db 6 (E195/E258/E259/E267/E269/E270; E154 owned-half only) · dc 6 (E174/E205/E235/E283/E343/E417-closed-no-code) · dd 9 (E111/E125/E127/E143/E144/E148/E149/E357/E386) · de 4 (E4/E17/E20/E175) · df 8 (E366/E370/E447-refuted/E448/E466/E486/E566/E629). Wave-2 **12 rows CLOSED** — dh 4 (E45/E74/E78c-with-evidence/E154 — both E154 remainder sites accounted: :5722 already-prefixed erratum, :8174 via de's E154c) · dj 2 (E342/E358; E319 stays PARTIAL, residue routed to ee) · di 6 (E272/E331/E356/E481/E565/E610) · dl 0 new (E686 continues, tranche-4 stamped, arms=23). **E3 → PARTIAL** (seam routed to r68 eb). **REFUSED stamps: E228, E214, E42b** (zero git trace; refusal notes at the rows). Errata healed at this close: dd's five triage ROW stamps (E127/E143/E144/E148/E149) were missed by `6682c9426` — closures were real (`4d58dadd7`, headings CLOSED), rows now stamped; and the disclosed +6 filemap-vs-ledger drift was reconciled by evidence to a chain-arithmetic defect born at the r64 close (triage header carries the verdict). E417 stale-prescription renote + E655 phantom re-issue note landed in the backlog at this close. Actionable **61 → 49** by chained arithmetic; **row census 55** — the +6 is the reconciled arithmetic defect, ledger note in the triage header. Wave-2 artifacts: `/home/sites/crush-r61-artifacts/{dh,dj,di,dl}/`.

## ROUND 66 — coverage shards, `/notices` goes live, the HOME-poison heals, and measure-first verdicts killed two duplicate lanes (2026-09-11)

**CLOSED at `61cde19c5`.** Base `ca2193942` (round-65 closeout). Six code lanes cc–ch + two verdict-only lanes cg/ch: **wave-1** `a59d8a547` (cc — E691) → `7f352c691` (cd — E670+E687+r65 seam bundle) → `5010c8bb2` (ce — E653-B `/notices`; picked **cherry-with-trailer from lane `0aebed27d`** — the lane SHA is NOT an ancestor, the git subject is the ancestry ledger) → re-pin `78a8f9d50`. **Wave-2** `4747b55f5` (cf — E655-seam base-red heal) → `31b6b94df` (ca — E657/E658/E659/E662) → `51e59798d` (cb — E686 tranche-3) → final re-pin `61cde19c5`. NOT pushed.

### THE FLOOR

| run | figure | domain |
|---|---|---|
| round base | 11,494 / 176,293 / 0F / 0E / 1S / exit 0 | LINKED @ `ca2193942`, cwd = sugar-crush (round-65 anchor) |
| wave-1 pin | 11,513 / 176,556 (`78a8f9d50`) | cc+cd+ce landed; ce alone +13T (its SlashDispatch attribution was errata'd — see Ledger) |
| **final merged (serial)** | **11,519 / 176,743 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0** (gate wall 558s) | **LINKED @ `61cde19c5`, cwd = sugar-crush — new anchor**; README headline + `suite-figure.json` agree; `durations.tsv` 491 rows; CI shard path K=4 conservation PASS (158s); Config filter **781T / 31,014A GREEN — the base-red is formally DEAD**; five-guard family 56T/4708A |

### LANE OUTCOMES

- **cc — E691 sharded coverage (lane `a2353660f` → pick `a59d8a547`).** `scripts/parallel-tests.sh` gained an opt-in `--clover` (default OFF; the OFF-path is byte-compatible, the usage-`sed` range and the "NO coverage" discipline line flipped IN-STEP) and new `scripts/merge-clover.php` unions per-shard clovers by summing per-line counts (crap scores recomputed line-only; file/project metrics recomputed from lines — single-class exact, multi-class conservative floor; **fail-closed**: exit 2 on any missing/malformed shard with nothing written; byte-identical re-merge via a generated-timestamp copied from shard 0), proven by a CI-invoked 66-check self-`test` (`scripts/test-merge-clover.php`, zero suite files, +0T/+0A) that runs FIRST in the job. ci.yml coverage job split: serial for the non-sugar-crush matrix, sharded K=min(nproc,4) for sugar-crush, the conservation-vs-figure check FAIL-CLOSED **before** the merge, and the merged artifact written to the SAME `sugar-crush/coverage.xml` path — Codecov/Codacy/split-repo/coverage-final consumers byte-untouched. Validation needed **pcov built from source**: the devbox's only driver, xdebug 3.2, SEGFAULTS nondeterministically under full-suite coverage — record so future lanes stop chasing it. K=4 pcov conservation PASS 11,494T/176,293A delta **+0/+0**; serial-vs-merged covered-line drift gross 3 lines, net +1 (`Renderer:3123` process-local imageCache eviction vs `Chat:12439-40`) — inherent process topology, zero merger loss.
- **cd — E670 + E687 + the r65 seam bundle (lane `5b61bdb31` → pick `7f352c691`).** E670 was measured ALREADY-LANDED in r63 (the `2f60e1696` hoist; its commit mis-cited E667) — the fix shipped was the comment citation → E670. E687 landed the pairing bd set up in r65: `__destruct()` teardown-total `error_log` (when `forkFailureCount > 0`, after the owner guard, before the resultDir early-return; message deliberately substring-disjoint from the two existing fork lines) with the census trio flipped IN-STEP (AgentWorkerPool roster row 2→3, aggregate prose TWENTY-THREE→TWENTY-FOUR; NUMBER_WORDS needed NO edit — "twenty-four" pre-existed) and `AgentWorkerPoolTeardownForkTotalTest` 4T/15A (helpers byte-copied from ForkFailureTest per drift discipline); the brief's "28→29" roster claim judged **FALSE-TO-TREE** (that's Bootstrap's channel-5 PREFIXED_WRITER_SITES census — untouched). Seam bundle: the keystone gained pid+grandchild liveness pins — a VACUITY gotcha surfaced (a naive bg `sleep` dies free when the session leader exits, `tty_hangup`→SIGHUP same-pgrp; the grandchild must be `(trap '' HUP; exec sleep 30) &` so the ignored disposition rides `exec` and only a group-kill can take it); the stuck branch now routes `ProcessReaper::escalate()` + `wait()` only when `$gone` — candy-pty `Child::wait()` is an unbounded `while(true)`, and a leaked live pty child HANGS phpunit shutdown (300s burn; timeout-`-s KILL` + `finally forceGone` leash makes mutations red-fast); `interactiveAvailable`'s function-local static moved to a private static property + testing-reset (function-local statics cannot be reset cross-method).
- **ce — E653 Shape B, the `/notices` command (lane `0aebed27d` → pick `5010c8bb2`).** `src/Commands/NoticesCommand.php` reports launch notices + drop count + narrowed-grant warnings through a pure `compose()` with SKELETON+entries line arithmetic (the forgery test COUNTS lines, scans no keyword — a message-scan oracle could be forged; every section header prints its count even at 0 with a `  - none` sentinel); args-tolerant `Chat.php` dispatch arm + permissions-inline transcript handler with NO `ob_start`/echo — deliberately stays out of the derived-stdout NoRawAnsiInTranscript census with no *Response twin; `Bootstrap::launchNoticesDropped()` public static (the dedup store is keyed by CLIPPED msg — an `array_values` returning bools bug caught by test); `reportField()` promoted private→public (E164 second-sibling precedent); `NoticesCommandTest` 12T/53A (helpers deliberately RENAMED vs the AgentManagerTest twins to dodge the DuplicatedTestHelperDrift name-scan); README roster + COMMANDS.md + ARCHITECTURE dispatch-arms numeral 22→23 flipped in-step (DocFigure live-pin). All five r65 census files verified UNCHANGED (554T/8323A filter green). Suite +13T / +200A churn from doc-pair assert-when-matching.
- **cf — the E655-seam Config base-red healed (lane `2aa474db1` → pick `4747b55f5`).** `NonInteractiveProviderFailureTest::setUp` snapshotted `HomeSandboxTrait` AFTER its ENV_KEYS unset loop (which includes HOME in `putenv`-unset form) → the trait captured `false` → restore re-unset HOME at the last tearDown write → every later test poisoned; the victim `RuntimeTest:4011` reported a misleading "sandbox did not come down". Tests-only, three files: (1) snapshot FIRST + re-arm after the loop — a **naive single-move above the loop was REJECTED**: it leaves body HOME unset and `Bootstrap::writeUserConfig` then writes the DEV'S real `~/.sugar-crush` via the passwd fallback (the trait's "snapshots first call, may re-arm" contract is the doc); (2) a 2-assert tripwire after `RuntimeTest:3957` so future mutations red at the poison site; (3) the scanner arm folded into EXISTING `OneSidedHomeSandboxTest` methods at a **DELIBERATE +0T/+10A** — any new test method bumps the live enumeration and reddens `Config\ReadmeSuiteFigureDriftTest` INSIDE the `--filter Config` window while README/suite-figure.json are merge-owned. Probe on all 51 trait users: 0 false positives.
- **ca — E657/E658/E659/E662 test-prose carry (lane `5b7348249` → pick `31b6b94df`).** Rule-4 symbol re-anchors across `src/Providers/` + `src/Runtime.php` prose and ~20 test dirs; the E659 guard-test rename landed with its `{@see}` + `SymbolCitationDriftTest` twin in one commit; E662's record-only re-derivation confirmed the autoload-then-miss shape still unreachable. **ERRATA (reviewer-caught):** the lane's ledger header claimed "31 fixed / 81 held" — the true split is **~77 fixed / ~35 held of 112 raw units**; the old figures double-counted wrapped multi-line citations. Honest record kept inline at the row, history unrewritten.
- **cb — E686 tranche-3 (lane `bf6f40202` → pick `51e59798d`).** Environment/Hooks/Providers/RuleLoader figure families judged and pinned — `DocFigureProseDriftTest` 12→18 arms. Campaign continues; tranche-4 carry = the HOOKS.md `CRUSH_*` 7×/8× roster claims → dynamic derivation.
- **cg — NO-OP (already-landed verdict).** The E653-B duplicate brief was caught before any code: `/notices` verified at tip (NoticesCommand 139 lines, dispatch `Chat.php:7295`/`:7464`, 12T/53A green) — zero diff, zero commit.
- **ch — NO-OP (CLOSED-superseded verdict).** E678's CLI/Bootstrap half measured ALREADY-LANDED: `mcpServerInventory()` (`Bootstrap.php:5384`) since `a4be8263f` — PREDATES the panel, so the entry's chronology was inverted (the panel `6220e5b29`/`a6768adde` consumed the same accessor; `--json` rows `b76fee3cd`). Parity is structural via one `mcpConfigDecision()` path; pins exist (`McpPanelTest:71`, `SlashDispatchTest:853-873`, `BinSugarcrushDispatchTest:1422`, `McpToolWiringTest:322/:343`). +0T/+0A.

### MEASURE-FIRST VERDICTS AT CLOSEOUT (all four adjudicated against git/tree at `61cde19c5` before stamping)

(a) **E261 CLOSED** — accessor `74dc2ab38` (r63) + teardown log via E687 `7f352c691`. (b) **E440 CLOSED** — seam `d0843c42c`/`96d87cdfb` + MCP mount `b76fee3cd` (E677) + LSP mount `858614688`/`ce87e4e1d` (E690); the loop-shape stays a documented E537 refusal, and the stamps say so. (c) **E43 stays PARTIAL** — the routed carrier E682 CLOSED, but it delivered the palette paint-parity rule; the adoption half (horizontal offset / per-block reflow-vs-clip toggle) has ZERO trace at tip (measured: no E43 marker, no fence-clip adoption, no scroll machinery in `sugar-crush/src/`) — left PARTIAL with the residual named, unowned. (d) **E42 CLOSED, heading reconciled** — backlog said [OPEN] while the ledger said PARTIAL for rounds; both halves now carry cited SHAs (`0373308a3` + the E665→E678→E689 chain).

### LESSONS

1. **The PTY-harness poison is a standing hazard, not a one-off** — a ctty-backed runner exports `LINES/COLUMNS` (measured 40×120 / 50×254 shapes) which deterministically reddens the lane-G keystone's piped-branch assertion (`TerminalSizeFallbackIsolationTest:167`). Full suites run through a PLAIN PIPE (the CI shape); the keystone still assumes unset env when piped — seam carried.
2. **Never move a sandbox snapshot blindly** — cf's naive one-line fix would have redirected the lanes' config writes into the developer's REAL home; the accepted shape is snapshot-first + re-arm-after, and the tripwire makes the next offender name its upstream poison.
3. **Cherry-pick twins ≠ lane originals** — `5010c8bb2` (master) is what ancestry means; `0aebed27d` (lane) lives only in the trailer and `git log`. Trust subjects and trailers, never "is the lane SHA an ancestor" checks.
4. **Reviewer arithmetic keeps earning its keep** — the ca lane's 31/81 ledger error and the ce lane's +13th-case misattribution were both caught by the closeout review, not the suite; errata ride inline.
5. **Measure-first verdicts are real cost savings** — cg and ch were both "already landed" once someone grepped before building; E43's verdict stopped a CLOSED stamp that the routed carrier did not actually support.

Ledger: **11 rows CLOSED** (E657, E658, E659, E662 | E670, E687 | E691 | E678-superseded | E261, E440, E42-measure-first), E653 re-stamped FULL CLOSED (Shape B — no count move, it left 111 as Shape A), E686 re-annotated (tranche-3, 18 arms; tranche-4 carries), E43 re-measured PARTIAL. Actionable **111 → 100** (file-map row census stamps 106 — a pre-existing +6 table-vs-ledger drift since the `0c61c0686` cut, disclosed in the filemap/triage headers, flagged for r67 reconciliation, not introduced this round). Round-67 lanes **da–df** composed at this close (§0-NOW-68 §2). Sandboxes `crush-lane-{cc,cd,ce,cf,ca,cb}` + r65 `crush-lane-{ba..be}` + r64 `{e671,ab..af}` all on disk — cleanup is a round-67 closeout chore; artifacts under `/home/sites/crush-r61-artifacts/{cc,cd,ce,cf,ca,cb,cg,ch}/`.

## ROUND 65 — the backlog headings mint at last, the /mcp panel and the palette truth land, and the permission drain flips FIVE censuses (2026-09-11)

**CLOSED at `0c61c0686`.** Base `e028f142c` (round-64 code tip; its closeout docs stack `c611441b5` beneath the round's own STEP 1). **Five lanes ba–be.** Chain: `25924d512` (STEP 1 — the 37 `### E652–E688` backlog headings minted from the triage ledger, the standing minting debt CLOSED; the mint's own in-file note cites `9940c9cfb`, the pre-merge SHA of the same change) → wave-1 picks `858614688 6220e5b29 a6768adde cf70cd0ae c477505fc 8222f9576 e18184e8a` (ba/bb/bc — **7 picks, zero conflicts, zero file overlap** across lanes) → drift fixes `bbebd033e`+`ce87e4e1d` → re-pin `74d16914e` (11,480 / 175,940) → wave-2 `8af65962d` (bd) + `d26dd4377` (bd review-2) + `09c08c481` (be) → final re-pin `0c61c0686`. NOT pushed.

### THE FLOOR

| run | figure | domain |
|---|---|---|
| round base | 11,462 / 175,615 / 0F / 0E / 1S / exit 0 | LINKED @ `e028f142c`, cwd = sugar-crush (round-64 anchor) |
| wave-1 merged | 11,480 / 175,940 / 1S | pinned at `74d16914e` after the two `DuplicatedTestHelperDriftTest` fixes; merge HOLD was correct — master waited at `e18184e8a` unre-pinned while the F3 blocker stood |
| **final merged (serial)** | **11,494 / 176,293 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0** (gate wall 559s, double-consistent) | **LINKED @ `0c61c0686`, cwd = sugar-crush — new anchor**; README headline + `suite-figure.json` agree; `durations.tsv` 489 rows; CI shard path K=4 conservation PASS (157s outer wall, slowest shard 156s) |

Predictions: wave-1 came in **+18 methods vs the +17 brief estimate** (one extra method somewhere — benign, the re-pin took ground truth); bd's **+6T** and be's **+8T** deltas hit exactly, continuing the additive-tests rule.

### LANE OUTCOMES

- **ba — E690 + E689 (lane `35b38ea5c`+`90747e04a`, review-fix `1c85b8498` → picks `858614688`/`6220e5b29`/`a6768adde`).** E690: `LspClient::pumpStderr()` fan-out mounted at all five `*For` dispatch entries (instanceof-gated so the interface never grows a pump method and in-memory fakes stay inert; `diagnostics()` skipped — pure cache read), pinned by `LspClientDispatchPumpTest` (post-settled chatterbox noise, empty-tail precondition, drained by the NEXT op, <2s, zero timers; mount-order row: the unknown-language throw precedes the drain). E689: `src/Tui/McpPanel` — a read-dominant transcript panel whose every row derives from `Bootstrap::mcpServerInventory()` (zero hard-coded rosters per E191; fixture tests randomise server names), mounted in the `McpAuthCommand` list arm so both the `/mcp` slash entry and the palette Toggle light up with no Chat/Renderer/KeyBinding edits; the trust toggle was declined — it would have invented a second persistence seam. **REVIEW: APPROVE-WITH-FIXES** — (1) the mount itself had NO pin → `SlashDispatchTest`'s mcp-auth row now asserts the panel header on BOTH spellings (argument-form so the popup cannot intercept; mutation-verified); (2) `McpPanel::line` clipped bytes → `Core\Util\Width::truncateMiddle` (display cells, codepoint-safe, UTF-8 row pins); (3) docblock hygiene. Drift fix `ce87e4e1d` (← `6c3168007`) aligned the copied `waitForFile` fail-message to canonical.
- **bb — E682 + E683 (lane `219bd908d`+`87508724f`, review-fix `d91a3c461` → picks `cf70cd0ae`/`c477505fc`/`8222f9576`).** E682: the composite chain's ONE rule — paint only what `Chat::update()` will route the next keystroke to — now holds: `Renderer::setPaletteAbandoned()` is computed per frame by `App::view()` from E666's `paletteIsAbandoned` predicate (reset in `finally`; standalone renders self-heal), and the palette link skips while set; suppression is paint-only, closing stays at the choke point; mutation-lifting paints the 8.8 KB ghost. E683: prose adoption-half TRUE → the `chatCtrlRunesYieldedToShell()` paragraph extended with the ADOPTION route; mouse pane-jump half measured **FALSE** (chrome zones are menu-only; `SelectPaneMsg` has a consumer and zero producers) → pinned dormant, NOT removed (never-remove rule), `mouse.pane` description de-overclaimed (`Focus that pane` → `Open the pane menu (palette)` — the label stays live via `pane:menu`). **REVIEW: APPROVE**; review-fix `8222f9576` = symmetric zone reset + alias-aware `SelectPaneMsg` needle; drift fix `bbebd033e` (← `4412fc39a`) canonical-qualified the copied `\ReflectionProperty`.
- **bc — Phase-9 layers B/C (lane `b7bb99ff5` → pick `e18184e8a`).** The opt-in `interactive` boolean on Bash, never a second tool: default-OFF byte-identical capture (the absent-pin flipped to an arrival+opt-in pin in-step); ON routes through a candy-pty PTY **this process** allocates — no setsid (the child needs a ctty and gets OURS), no-askpass design holding verbatim on the pty path; a program that blocks for input is group-TERMed at the 8s idle ceiling (exit 124 + refusal), a host without the pty mechanism gets a clean 126 before anything starts. The design's `Chat::withLaunchNotices` refusal shape shipped as a **justified per-call ToolResult deviation** — notices drain once at construction; routing a mid-session refusal through them would invent exactly the forbidden second seam. +5 methods in `InteractivePromptContainmentTest`; Containment filter 2012T/17120A. **E692 dispositioned in-lane**: the `EngineBackend` in-process `pcntl_fork` is exec-free — no argv/PATH/interactive surface — therefore OUT of `ProcessContainment`'s exec-spawn remit; the scope call is recorded as a comment at the fork site. **REVIEW: APPROVE.**
- **bd — E653 Shape A (lane `03912ee13`, review-2 `6b52bf7e5` → picks `8af65962d`/`d26dd4377`).** The aggregated permission-warning drain at the `chat()` tail: stderr gets every `narrowedGrantWarnings()` whole and once; the transcript gets ≤2 rows via a greedy pair-packer to the 400-char launch-notice budget, fail-soft to whole sentences on collector drift. New `PermissionWarningDrainTest` 6T/47A, dynamic N/K/M read back from the child's own collector (E191). **The census flip set was FIVE files, not four** — the PROPOSAL missed `BootstrapLaunchFormatConstantsTest` (any new `sprintf` in `Bootstrap.php` obliges it: calls 13→15, two NAMED_FORMATS rows, one METHOD_LITERALS allowlist). Census-mandated **out-of-touch-list prose** flipped in-step and declared: `Chat.php` docblock, `McpToolWiringTest`, `docs/SETTINGS.md`. **REVIEW: APPROVE + 2 guard misses** at the full-suite gate — GlobDialectDifferential (the tail-regex literals `'s?'` inflated the harvested pair-count 131,040→131,765 — remedy: **re-shape to `'(?:s)?'`**, identical PCRE, non-capturing, guard number stays TRUE) and ChildWallClockBudget (the e2e child armed 60s == parent limit — remedy: measured 40s = sibling's 20s headroom × two launches, derivation commented).
- **be — E686 tranche-2 (lane `85a20c50c` → pick `09c08c481`).** 27 figure claims across 14 files judged against code: **16 TRUE pinned** in 8 new `DocFigureProseDriftTest` arms (class now 12 arms: HttpClientDefaults 15s/120s×3/MCP-30/hour; TransientFailure 500ms; LSP 16×8192@20ms→6 MB/s + 30.0/10000; the 64 KiB stderr-tail FAMILY cross-equality; Stdio SIXTY↔60.0↔MCP.md; agents reap 20×5=100 + Mailbox literals + TaskList 10ms/5.0/busyTimeout; Workflow 5/3600 incl. the 300≠3600 domain assert; ARCHITECTURE live counters), **3 FALSE fixed prose-only** in `docs/ARCHITECTURE.md` (11→12 built-ins after TaskTool; the 219-line and 4,253/70 figures de-digitalised, all-static kept live-pinned), **6 HELD** as measured/external-labeled. Zero executable-code lines; mutation-proven (900ms/32-KiB probes reddened exactly arms B+D; cp-restored per the lane-ab lesson). Gotcha filed: `ParsedArgs` is NOT PSR-4 autoloadable (shares `ArgvParser.php`). **REVIEW: APPROVE.**

### REVIEWS AND PROCESS FINDINGS

The tree-wide-guard miss pattern struck **twice more** (bd: GlobDialect corpus + ChildWallClock — the third occurrence after r64's ac/ad pair), and the wave-1 merge itself HODLED on `DuplicatedTestHelperDriftTest` (two drift rows intrinsic to new lane files — an unqualified `ReflectionProperty` and a reworded `waitForFile` format string — deterministic at each lane's own tip, invisible to their targeted filters). Both were fixed by the OWNING lanes' review-2 commits, not by merge-time band-aids. The bd case proved the flip-count lesson: **the census trio/quartet/sextet doctrine must now name FIVE census files for Bootstrap work**, and a census scan that obliges a numeral edit outside the lane's touch-list is a legitimate in-step flip, not an ownership violation — provided it is numeral-only, zero behavior, and declared. The live-guard re-measure rode the final re-pin: `PathGlob.php:51`'s doc-block corpus figure moved to 131,765 (365×361) only after bd re-shaped its literals, so the guard's number is true rather than rubber-stamped.

### LESSONS

1. **The guard-family filter mandate is PROVEN, not theoretical** — three consecutive rounds where targeted filters missed tree-wide guards (r64 ×2, r65 ×2). From round 66 EVERY builder AND reviewer brief carries: `SwallowingCatchCensus|OneSidedHomeSandbox|DuplicatedTestHelperDrift|GlobDialectDifferential|ChildWallClockBudget`.
2. **Census-mandated out-of-touch-list prose edits are legitimate in-step flips** — flip the numeral, change zero behavior, declare it in REPORT (bd's Chat.php/SETTINGS.md/McpToolWiringTest set). And enumerate EVERY census that scans a lane file before writing the brief — `BootstrapLaunchFormatConstantsTest` was the fifth census nobody predicted.
3. **Re-shape the corpus before re-pin** — when a new string literal is harvest-shaped (glob dialects), prefer the token-variant with identical meaning (`'(?:s)?'` for `'s?'`) so the guarded figure stays TRUE; re-pinning a guard to your own residue converts a tripwire into a rubber stamp.

Ledger: **6 CLOSED** (E653 — Shape A, Shape B follow-up noted; E682, E683, E689, E690; E692 CLOSED-**with-decision**), E686 re-annotated (tranche-2 landed, 12 arms; tranche-3 carries), E691 untouched by design (coverage job still serial — round-66 candidate), Phase 9 stays **PARTIAL** (layers B/C landed; the composited in-pane surface + E62's permission-gate bar remain). **`### E652–E688` headings minted — the minting debt is CLOSED.** Actionable **117 → 111**. Sandboxes `crush-lane-{ba,bb,bc,bd,be}` and artifacts under `/home/sites/crush-r61-artifacts/{merge-r65-w1,merge-r65-w2,ba,bb,bc,bd,be}/`.
