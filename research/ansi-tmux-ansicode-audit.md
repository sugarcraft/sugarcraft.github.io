...
- **SCS / DEC Special Graphics missing in every consumer** — `ESC(0jqx` prints literal `jqxk` (ansicode:222-249 baseline); `ESC # 8` DECALN **mis-executes as DECRC** (cursor teleport observed). **Major** (candy-vt, candy-freeze, sugar-spark) ✅ *candy-vt half since resolved* — `ScreenHandler::escDispatch()` now gates on the intermediate, `designate()`/`Charsets` translate G0-G3, and `ESC # 8` dispatches as esc(0x38,0x23) — and since PR #1431 `escDispatch()` routes that intermediate/final pair to `displayAlignmentTest()`, so the sequence now executes the alignment fill rather than being ignored or mis-executed as DECRC (the emitter→emulator round trip, and the inertness of the `ESC [ # 8` misquote, are pinned by `candy-vcr/tests/CoreEmitterRoundTripTest.php` and `candy-vt/tests/Handler/DecalnWireTest.php`); candy-freeze/sugar-spark left to their own track.
